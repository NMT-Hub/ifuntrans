...
from deep_translator import (DeeplTranslator, GoogleTranslator,
                             LibreTranslator, LingueeTranslator,
                             MicrosoftTranslator, MyMemoryTranslator,
                             PapagoTranslator, PonsTranslator, QcriTranslator,
                             YandexTranslator)

from ifuntrans.api import app
from ifuntrans.api.models.requests import (DeeplRequest, GoogleRequest,
                                           LibreRequest, LingueeRequest,
                                           MicrosoftRequest, MyMemoryRequest,
                                           PapagoRequest, PonsRequest,
                                           QcriRequest, YandexRequest)
from ifuntrans.api.models.responses import TranslationResponse
from ifuntrans.utils import get_summary, get_translation
...
@app.post(f"/{GOOGLE}/", summary=get_summary(GOOGLE), response_model=TranslationResponse)
...
@app.post(f"/{MICROSOFT}/", summary=get_summary(MICROSOFT), response_model=TranslationResponse)
def microsoft_translate(r: MicrosoftRequest):
    ...
    """
    t = MicrosoftTranslator(source=r.source, target=r.target, region=r.region, api_key=r.api_key)
    return get_translation(t, r.text)
...
@app.post(f"/{MYMEMORY}/", summary=get_summary(MYMEMORY), response_model=TranslationResponse)
...
@app.post(f"/{PAPAGO}/", summary=get_summary(PAPAGO), response_model=TranslationResponse)
...
@app.post(f"/{YANDEX}/", summary=get_summary(YANDEX), response_model=TranslationResponse)
...
@app.post(f"/{LINGUEE}/", summary=get_summary(LINGUEE), response_model=TranslationResponse)
